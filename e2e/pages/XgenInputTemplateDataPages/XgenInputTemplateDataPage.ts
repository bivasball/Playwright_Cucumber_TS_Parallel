...
export default class xgenDataInputPage {
    ...
    AddInputTemplate() {
        return `//div[contains(@class,'MuiGrid2-direction-xs-row MuiGrid2-spacing-xs-3')]/div[1]//a[@iconcolor="create"]`;
    }
    AddPlusIconRecord() {
    ...
    async cleanUpTheInputData(jsondata: any) {
        ...
        let presentOrNotRow = await playwrightWrapper.getTheRowNumberFromInputPage(inputname);
        console.log("Present or not, if present the row number is ==========", presentOrNotRow);
        if (presentOrNotRow !== 0) {
            console.log("Deleting the InputTemplate as per the request.");
            await playwrightWrapper.deleteTheTemplate(presentOrNotRow);
        }
    }
    ...
    async addRecordSaveCommit() {

        await globalAction.waitAndClick(this.AddInputTemplate());
        await globalAction.waitAndClick(this.AddPlusIconRecord());
        await globalAction.waitAndClick(this.AddRecord1());
        await globalAction.waitAndClick(this.AddPlusIconRecord());
        await globalAction.waitAndClick(this.AddRecord2());
        await globalAction.waitAndClick(this.Saverecord());
