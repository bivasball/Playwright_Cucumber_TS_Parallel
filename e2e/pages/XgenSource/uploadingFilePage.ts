...
async uploadfiles(jsonData: any) {
    ...
    await fixture.page.waitForSelector('role=radio[name="New"]', { state: "visible", timeout: TIMEOUT });
    ...
    await fixture.page.waitForSelector('role=textbox[name="Source Name"]', { state: "visible", timeout: TIMEOUT });
    ...
    await fixture.page.getByRole('textbox', { name: 'Source Name' }).fill(sourceName);
    fixture.logger.info(`Filled 'Source Name' textbox with: ${sourceName}`);

    // Wait for and fill the "Source Description" textbox
    await fixture.page.waitForSelector('role=textbox[name="Source Description"]', { state: "visible", timeout: TIMEOUT });
    fixture.logger.info("Waiting for 'Source Description' textbox to be visible...");
    await fixture.page.getByRole('textbox', { name: 'Source Description' }).click();
    fixture.logger.info("Clicked on 'Source Description' textbox.");
    await fixture.page.getByRole('textbox', { name: 'Source Description' }).fill(sourceDescp);
    fixture.logger.info(`Filled 'Source Description' textbox with: ${sourceDescp}`);

    // Verify the "File Upload" dialog
    await fixture.page.waitForSelector('role=dialog', { state: "visible", timeout: TIMEOUT });
    ...
    // Verify labels
    await fixture.page.waitForSelector('#srcname-label', { state: "visible", timeout: TIMEOUT });
    fixture.logger.info("Waiting for 'Source Name *' label to be visible...");
    await expect(fixture.page.locator('#srcname-label')).toContainText('Source Name *');
    fixture.logger.info("Verified 'Source Name *' label is visible.");

    await fixture.page.waitForSelector('#srcdesc-label', { state: "visible", timeout: TIMEOUT });
    fixture.logger.info("Waiting for 'Source Description *' label to be visible...");
    await expect(fixture.page.locator('#srcdesc-label')).toContainText('Source Description *');
    fixture.logger.info("Verified 'Source Description *' label is visible.");

    // Select the file format
    await fixture.page.waitForSelector('role=combobox[name="File Format"]', { state: "visible", timeout: TIMEOUT });
    ...
    const fileInput = fixture.page.locator(`//input[@id='upload-import-file' and @name='file']`);
    ...
    playwrightWrapper.loadingWebPage();
    await fixture.page.waitForSelector('role=dialog', { state: "visible", timeout: TIMEOUT });
    fixture.logger.info("Waiting for file upload confirmation in the dialog...");
    await expect(fixture.page.getByRole('dialog')).toContainText(`Name: ${fileNames}`);
    fixture.logger.info(`Verified uploaded file name in the dialog: ${fileNames}`);

    // Click the "Save" button
    await fixture.page.waitForSelector('role=button[name="Save"]', { state: "visible", timeout: TIMEOUT });
    ...
    playwrightWrapper.loadingWebPage();
    await fixture.page.waitForSelector('#root', { state: "visible", timeout: TIMEOUT });
    fixture.logger.info("Waiting for the source name to appear on the board...");
    await expect(fixture.page.locator('#root')).toContainText(sourceName);
    fixture.logger.info(`The filename entered is present on the board: ${sourceName}`);
}
