Feature: To upload the files like CategoryForecast.xlsx, company_data.csv and Company Survey.xlsx
    ...
    @xgen @e2e @uploadFiles @upload_04 @unik
    Scenario Outline: To upload for Unique Sourcename , CategoryForecast.xlsx file, after cleanup activity
        Given User logs into the application with username and password and sees the message and selects the subscription "<loginData>"
        Then User navigates through the "Home" and "Spaces" pages, selects the "Fitness" radio option, and sees the welcome message "<loginData>"
        When user navigate to the Connect page and setup unique source name for CategoryForecast "<data>"
        Then user cleanup and upload the file. "<data>"
            And User logout from the application "Logout"
        Examples:
            | loginData                       | data                                  |
            | LoginData-testdataForLoginUser1 | XgenSourceData-testdataForecastUnique |



    @xgen @e2e @uploadFiles @upload_05 @unik
    Scenario Outline: To upload for Unique Sourcename , company_data.csv file, after cleanup activity
        Given User logs into the application with username and password and sees the message and selects the subscription "<loginData>"
        Then User navigates through the "Home" and "Spaces" pages, selects the "Fitness" radio option, and sees the welcome message "<loginData>"
        When user navigate to the Connect page and setup unique source name for Company Data "<data>"
        Then user cleanup and upload the file. "<data>"
            And User logout from the application "Logout"
        Examples:
            | loginData                       | data                            |
            | LoginData-testdataForLoginUser1 | XgenSourceData-CompayDataUnique |



    @xgen @e2e @uploadFiles @upload_06 @unik
    Scenario Outline: To upload for Unique Sourcename ,Company Survey.xlsx file, after cleanup activity
        Given User logs into the application with username and password and sees the message and selects the subscription "<loginData>"
        Then User navigates through the "Home" and "Spaces" pages, selects the "Fitness" radio option, and sees the welcome message "<loginData>"
        When user navigate to the Connect page and setup unique source name for Company Survey "<data>"
        Then user cleanup and upload the file. "<data>"
            And User logout from the application "Logout"
        Examples:
            | loginData                       | data                               |
            | LoginData-testdataForLoginUser1 | XgenSourceData-CompanySurveyUnique |
